  ...
  useEffect(() => {
    ...
    const fetchApi = async () => {
      try {
        ...
        const result = res.weather;
        const status = result[0].main;
        ...
        if (status === 'Clouds' || status === 'Haze') {
          setWeatherStatus(
            <div>
              <i className='fas fa-cloud'></i> <h3>Cloudy</h3>
            </div>
          );
        } else if (status === 'Rain') {
          setWeatherStatus(
            <div>
              <i class='fas fa-cloud-rain'></i> <h3>Rainy</h3>
            </div>
          );
        } else {
          setWeatherStatus(
            <div>
              <i className='fas fa-sun'></i> <h3>Sunny</h3>
            </div>
          );
        }
      } catch (err) {
    ...
    <>
      <div className='box'>
        ...
        {!city ? (
          ...
          <div className='info'>
            <div className='desc'>{weatherStatus}</div>
